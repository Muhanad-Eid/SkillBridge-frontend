...
export default function EvidenceRequestWorkspacePage() {
  ...
  const counts = useMemo(() => ({ open: requests.filter((item) => item.status === "Open").length, responded: requests.filter((item) => item.status === "Responded").length, resolved: requests.filter((item) => item.status === "Resolved").length }), [requests]);

  async function load() {
    setIsLoading(true); setError("");
    try { setRequests(await getEvidenceActionRequestsAsync()); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to load evidence requests."); }
  ...
  return <section className="page evidence-request-workspace">
    <PageHeader title="Evidence Requests" />
    <p className="evidence-request-intro">Focused proof requests that point to the real work. A request never substitutes for evaluation, approval, or issuance.</p>
    ...
    <div className="evidence-request-layout"><section className="evidence-request-list"><DataState isLoading={isLoading} error="" empty={!isLoading && requests.length === 0} emptyTitle="No evidence requests" emptyDescription="Requests will appear here when work needs a focused proof response." />{requests.map((item) => <button type="button" className={selected?.id === item.id ? "selected" : ""} key={item.id} onClick={() => setSelected(item)}><div><span>{item.requestType.replace(/([A-Z])/g, " $1").trim()}</span><strong>{item.title}</strong><small>{item.opportunityTitle} · {isParticipant ? item.requesterName : item.participantName}</small></div><StatusBadge tone={item.status === "Resolved" ? "green" : item.status === "Responded" ? "blue" : "amber"}>{item.status}</StatusBadge></button>)}</section>
